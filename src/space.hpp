...
public:
  ...
  Space()
  {
    ...
    gun->SetShotDirection(QVector2D(0, 1));
    ...
    for(int i = 0; i < m_amountAlien; i++)
    {
      ...
      m_alien.back()->SetSize(QSize(128, 128));
    }

    for(int i = 0; i < m_amountObstacle; i++)
    {
      m_obstacle.push_back(std::move(std::shared_ptr<Obstacle>(new Obstacle)));
      m_obstacle.back()->SetPosition(QVector2D(400*i + 120, 150));
      m_obstacle.back()->SetSize(QSize(200, 90));
    }
